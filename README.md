...
Since we only observe students above a threshold, we're dealing with truncated distributions—and the published statistics are systematically misleading. This project develops a Maximum Likelihood Estimation (MLE) framework to recover the hidden population parameters (μ, σ) from the published truncated statistics, then uses these recovered parameters to build a Monte Carlo simulation of the full admissions process.

The analysis reveals three critical findings. First, published averages overstate competitiveness by 100-300 points for lower-tier students, creating a systematic discouragement effect that undermines the equity goals of the tier system. Second, the data expose a fundamental bifurcation: elite schools (Payton, Northside, Whitney Young, Jones, Lane Tech) operate with extraordinarily tight Tier 4 competition (σ ≈ 3-25 points), while regional schools show heterogeneous applicant pools (σ ≈ 60-100 points)—these are not variations within one system but two distinct admissions regimes. Third, several regional schools exhibit inverted tier patterns where Tier 4 cutoffs fall below Tier 1 cutoffs, revealing how rational student behavior and geographic sorting interact with policy design in unexpected ways. The tier system creates advantages ranging from 40 to 186 points between Tier 1 and Tier 4 students—equivalent to one to five letter grades—raising questions about the appropriate magnitude of equity interventions in high-stakes admissions.
